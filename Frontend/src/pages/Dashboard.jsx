...
export default function Dashboard() {
  ...
  const metrics = [
    {
      title:   "Total Funding",
      value:   totalFunding >= 10_000_000
                 ? `₹${(totalFunding / 10_000_000).toFixed(2)} Cr`
                 : totalFunding >= 100_000
                   ? `₹${(totalFunding / 100_000).toFixed(2)} L`
                   : totalFunding >= 1_000
                     ? `₹${(totalFunding / 1_000).toFixed(0)}K`
                     : `₹${totalFunding}`,
      change:  totalFunding >= 100_000
                 ? `₹${(totalFunding / 100_000).toFixed(1)}L raised`
                 : `₹${(totalFunding / 1_000).toFixed(0)}K raised`,
    ...
    },
    {
      ...
      trend:   "up",
      icon:    <BusinessCenterIcon sx={{ fontSize: 30, display: "block", m: 0, p: 0 }} />,
      color:   GREEN_COLORS.darkGreen,
      bg:      `linear-gradient(135deg, ${GREEN_COLORS.darkGreen} 0%, ${GREEN_COLORS.darkGreen}dd 100%)`,
    },
    {
      ...
      <Grid container spacing={3} mb={4}>
        {metrics.map((metric, index) => (
          <Grid key={index} size={{ xs: 12, sm: 6, md: 4, lg: 2 }}>
            ...
            >
              <CardContent sx={{ position: "relative", p: "12px 16px !important" }}>
                <Box
                  sx={{
                    position: "absolute",
                    top: 12,
