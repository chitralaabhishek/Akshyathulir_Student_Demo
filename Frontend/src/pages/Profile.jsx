...
function App() {
  ...
  const [loadingPhoneCodes, setLoadingPhoneCodes] = useState(false);
  ...
  useEffect(() => {
    const fetchPhoneCountries = async () => {
      ...
      try {
        ...
        setPhoneCountries(formatted);

      } catch (error) {
        console.error("Phone country fetch error:", error);
      } finally {
  ...
  const handleInputChange = (field) => (event) => {
    ...
    if (
      ["currentTeamSize", "maleCount", "femaleCount", "numberOfBranches"]
        .includes(field) &&
      value !== "" &&
      Number(value) < 0
    ) {
    ...
    // Clear field error if user fixes it
    if (errors[field]) {
      setErrors((prevErr) => ({
        ...prevErr,
        [field]: "",
                        ...
                        {phoneCountries.map((c) => (
                          <MenuItem key={c.name} value={c.name}>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                              <img src={c.flag} width="20" height="15" alt={c.name} />
                              {c.dialCode}
                            </Box>
                          </MenuItem>
                        ))}
              ...
              <TextField
                ...
                onChange={(e) => {
                  ...
                  if (val && !EMAIL_REGEX.test(val)) {
                    setErrors((prev) => ({
                      ...prev,
                      founderEmail: "Enter a valid email address",
                    }));
                  }
                }}
                        ...
                        onChange={(e) => {
                          ...
                          if (selected) {
                            setFormData((prev) => ({
                              ...
                              founderPhoneCode: selected.dialCode,
                              founderPhone: "",
                            }));
                          }
                        }}
